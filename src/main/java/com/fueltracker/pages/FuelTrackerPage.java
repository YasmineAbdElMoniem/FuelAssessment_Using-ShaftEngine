...
public class FuelTrackerPage extends PageBase {
    ...
    public FuelTrackerPage(SHAFT.GUI.WebDriver driver) {
        super(driver);
    }
    ...
    public Boolean checkValidationMessageAppearForEveryTextField() {
        List<WebElement> inputFieldsList = driver.getDriver().findElements(listOfFields);
    ...
    public Boolean checkDateTimeFieldValidation() {
        String tooltip = driver.getDriver().findElement(dateTimeField).getAttribute("validationMessage");
    ...
    public Boolean checkTheRecordIsAddedSuccessfully(double fuelInGallonsToCheck, double fuelCostToCheck, int companyIdToCheck) {
        ...
        List<WebElement> columnsNames = driver.getDriver().findElements(tableColumnsNames);

        // Iterate through columns Names
        for (int i = 0; i <= columnsNames.size(); i++) {
            // Get all values for each column name
            List<WebElement> columnValues = driver.getDriver().findElements(tableColumnsValues);
